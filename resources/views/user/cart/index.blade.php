...
<div class="container">
    <table class="table" style="background:#fff;">
        ...
        <tr>
            ...
            <td class="product__desciption"> {{number_format($cart->products->first()->gia)}}</td>
            <td>{{$cart->soluong}} </td>
            <td> {{number_format($cart->products->first()->gia * $cart->soluong)}}</td>
        </tr>
        @endforeach
    </table>
    <div class="payment_cart_right">
        <button id="payment_Product_checked" class="btn btn-danger" value="0" name="submit"> Mua Hàng</button>
        <button type="button" class="btn btn-danger" id="submit-del-products" data-user-id="{{ $userId }}">Xoá</button>
    </div>
    @endif
</div>
@endsection
